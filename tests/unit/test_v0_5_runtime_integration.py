...
from ast2python.diagnostics import REFERENCE_HISTORY_UNSUPPORTED, REQUEST_SECURITY_CAPTURE_UNSAFE
...
def decl(kind="indicator"):
    return {"kind": "DeclarationStatement", "script_type": kind, "call": {"kind": "CallExpr", "callee": {"kind": "Identifier", "name": kind}, "arguments": [{"kind": "Argument", "name": None, "value": {"kind": "Literal", "literal_type": "string", "value": "v05"}}]}}
...
def test_date_helpers_lower_to_runtime_timefunc():
    p = program([
        {"kind": "VarDeclaration", "name": "y", "span": {"start_line": 3, "start_col": 1}, "initializer": call("year", [arg(ident("time"))])},
...
def test_reference_array_history_fails_before_runtime_and_copy_warns():
    ...
    bad = program([
        {"kind": "VarDeclaration", "name": "a", "type_ref": {"kind": "TypeRef", "name": "array"}, "span": {"start_line": 3, "start_col": 1}, "initializer": arr_new},
        {"kind": "VarDeclaration", "name": "prev", "span": {"start_line": 4, "start_col": 1}, "initializer": {"kind": "HistoryRefExpr", "base": ident("a"), "offset": lit(1)}},
    ])
    with pytest.raises(TypeResolutionError):
        translate_ast(bad, module_name="array_history")

    ok = program([
        {"kind": "VarDeclaration", "name": "a", "span": {"start_line": 3, "start_col": 1}, "initializer": arr_new},
...
def test_request_security_capture_safety_warning_and_strict_error():
    p = program([
        {"kind": "VarDeclaration", "name": "basis", "span": {"start_line": 3, "start_col": 1}, "initializer": ident("close")},
        {"kind": "VarDeclaration", "name": "htf", "span": {"start_line": 4, "start_col": 1}, "initializer": call("request.security", [arg(lit("AAPL", "string")), arg(lit("D", "string")), arg(ident("basis"))])},
    ])
...
def test_source_map_fixture_coverage_is_at_least_95_percent():
    ...
    for fixture in fixtures.glob("*.ast.json"):
        result = translate_ast(json.loads(fixture.read_text(encoding="utf-8")), module_name=fixture.stem)
...
def test_cli_smoke_runs_generated_code_against_sample_bars(tmp_path):
    fixture = Path(__file__).resolve().parents[1] / "fixtures" / "ast" / "minimal_indicator.ast.json"
    out = tmp_path / "generated"
    subprocess.run([sys.executable, "-m", "ast2python.cli.main", "translate", str(fixture), "-o", str(out), "--module-name", "smoke_min"], check=True)
    proc = subprocess.run([sys.executable, "-m", "ast2python.cli.main", "smoke", str(out / "smoke_min.py")], check=True, text=True, capture_output=True)
