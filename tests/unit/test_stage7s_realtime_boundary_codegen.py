    ...
    return {
        ...
        'declaration': {
            ...
            'call': {
                ...
                'arguments': [
                    {'kind': 'Argument', 'name': None, 'value': _lit('Stage7I local realtime skeleton', 'string')},
                    {'kind': 'Argument', 'name': 'calc_on_every_tick', 'value': _lit(True, 'bool')},
                ],
            },
        },
        'items': [
            {'kind': 'VarDeclaration', 'mode': 'varip', 'name': 's3t_varip_tick', 'initializer': _lit(0, 'int')},
        ],
    }


def test_stage7i_realtime_strategy_codegen_rejects_by_default_and_allows_local_simulation_only() -> None:
    program = _program()
