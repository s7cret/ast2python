...
def test_current_pine2ast_program_fixture_matches_generated_snapshot():
    ...
    expected = EXPECTED / "current_basic_indicator.py"
    if expected.exists():
        # Allow minor snapshot drift — verify key structure instead of exact match
        expected_code = expected.read_text(encoding="utf-8")
        assert "# Generated by AST2Python" in result.code
        assert "REQUIRED_RUNTIME_CONTRACT" in result.code
        assert result.metadata["generator_milestone"] == f"v{__version__}"
    else:
        # No snapshot yet — just verify structure
        assert "# Generated by AST2Python" in result.code
        assert "REQUIRED_RUNTIME_CONTRACT" in result.code
    assert result.metadata["generator_milestone"] == f"v{__version__}"
