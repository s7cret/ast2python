...
def test_function_notation_and_dot_share_receiver_types_and_state(version, imported, kind):
    ...
    fixtures = {
        ...
        "array": (
            "a=array.new<float>(2,7)",
            "export method get(array<float> self)=>self.get(1)",
            7,
        ),
        "map": (
            'a=map.new<string,int>()\na.put("key",7)',
            'export method get(map<string,int> self)=>self.get("key")',
            7,
        ),
        "matrix": (
            "a=matrix.new<int>(2,2,7)",
            "export method get(matrix<int> self)=>self.get(0,1)",
            7,
        ),
        "udt": (
            f"a={ns}P.new(7)",
            "export type P\n    int n=0\nexport method get(P self)=>self.n",
            7,
        ),
        "enum": (
            f"a={ns}Side.down",
            "export enum Side\n    up\n    down\nexport method get(Side self)=>self==Side.up ? 1 : -1",
            -1,
        ),
    }
...
def test_transitive_explicit_calls_and_private_bare_helpers_do_not_leak(version):
    libs = {
        ...
        "user/Lib/1": library(
            "import u/D/1 as d\nmethod hidden(simple int self)=>d.deep(self)+1\nexport method outer(simple int self)=>hidden(self)",
            version=version,
        ),
    }
...
@pytest.mark.parametrize(
    "fault", ["missing_capability", "extra_capability", "receiver", "parameter", "actual_type"]
)
def test_explicit_receiver_bundle_replay_rejects_forged_facts(fault):
    ...
    if fault == "missing_capability":
        ...
        if fault == "receiver":
            call["receiver_type"] = "float"
        elif fault == "parameter":
            call["arguments"][0]["parameter_name"] = "n"
        else:
            call["arguments"][0]["actual_type"] = "string"
