...
from tests.test_rc6_input_metadata import run_source, compile_source
from tests.test_nominal_language_execution import source
from tests.test_locked_library_execution import compile_linked, library, script, runtime_for, advance
...
@pytest.mark.parametrize('expr,expected', [('5%2',1),('-5%2',1),('5%-2',-1),('-5%-2',-1),
                                           ('-5.5%2.0',0.5),('5.5%-2.0',-0.5)])
def test_modulo_default_and_runtime_have_one_floor_contract(version, expr, expected):
    kind = 'float' if '.' in expr else 'int'
    assert trace(f'n=input.{kind}({expr})\nplot(n)\nplot({expr})', version) == [expected,expected]*3


@pytest.mark.parametrize('version', [5, 6])
@pytest.mark.parametrize('expr,expected', [('false==0',False),('true==1',False),
...
def test_legacy_int_division_does_not_round_through_float(number,divisor):
    ...
    assert trace(f'n=input.int({number}/{divisor})\nplot(n=={expected})\nplot(({number}/{divisor})=={expected})',5)==[True,True]*3
...
def test_color_components_and_versioned_palette(version,red):
    assert trace('plot(color.r(color.red))\nplot(color.g(#123456))\nplot(color.b(#123456))\nplot(color.t(#12345600))',version,closes=(1,)) == [red,52.0,86.0,100.0]
...
def test_imported_same_named_enum_titles_are_distinct(version):
    libs={'u/A/1':library('export enum E\n    one="Alpha"','A',version),
          'u/B/1':library('export enum E\n    one="Beta"','B',version)}
    compiled,_=compile_linked(script('plot(str.tostring(a.E.one)=="Alpha")\nplot(str.tostring(b.E.one)=="Beta")',
                                    'import u/A/1 as a\nimport u/B/1 as b',version),libs)
    r,cls=runtime_for(compiled)
    assert advance(r,cls,[1,2])==[True,True]*2


@pytest.mark.parametrize('body',[
    'plot(str.contains("a",1))', 'plot(str.substring("abc","1"))',
    'plot(str.replace("a","a"))', 'plot(nz("bad"))',
